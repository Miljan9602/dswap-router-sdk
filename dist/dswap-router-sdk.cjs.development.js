...
var IApproveAndCall_json = require('@dragonswap/swap-router-contracts/artifacts/contracts/interfaces/IApproveAndCall.sol/IApproveAndCall.json');
var dswapV3Sdk = require('@miljan9602/dswap-v3-sdk');
var IMulticallExtended_json = require('@dragonswap/swap-router-contracts/artifacts/contracts/interfaces/IMulticallExtended.sol/IMulticallExtended.json');
var IPeripheryPaymentsWithFeeExtended_json = require('@dragonswap/swap-router-contracts/artifacts/contracts/interfaces/IPeripheryPaymentsWithFeeExtended.sol/IPeripheryPaymentsWithFeeExtended.json');
var ISwapRouter02_json = require('@dragonswap/swap-router-contracts/artifacts/contracts/interfaces/ISwapRouter02.sol/ISwapRouter02.json');
...
var ZERO_PERCENT = /*#__PURE__*/new dswapSdkCore.Percent(ZERO);
var ONE_HUNDRED_PERCENT = /*#__PURE__*/new dswapSdkCore.Percent(100, 100);
...
var ApproveAndCall = /*#__PURE__*/function () {
  ...
  ApproveAndCall.encodeCallPositionManager = function encodeCallPositionManager(calldatas) {
    ...
    if (calldatas.length == 1) {
      ...
      var encodedMulticall = dswapV3Sdk.NonfungiblePositionManager.INTERFACE.encodeFunctionData('multicall', [calldatas]);
  ...
  ApproveAndCall.encodeAddLiquidity = function encodeAddLiquidity(position, minimalPosition, addLiquidityOptions, slippageTolerance) {
    ...
    if (isMint(addLiquidityOptions)) {
      return ApproveAndCall.INTERFACE.encodeFunctionData('mint', [{
        ...
        amount0Min: dswapV3Sdk.toHex(amount0Min),
        amount1Min: dswapV3Sdk.toHex(amount1Min),
      ...
      return ApproveAndCall.INTERFACE.encodeFunctionData('increaseLiquidity', [{
        ...
        amount0Min: dswapV3Sdk.toHex(amount0Min),
        amount1Min: dswapV3Sdk.toHex(amount1Min),
        tokenId: dswapV3Sdk.toHex(addLiquidityOptions.tokenId)
      }]);
    }
  };
...
var MulticallExtended = /*#__PURE__*/function () {
  ...
  MulticallExtended.encodeMulticall = function encodeMulticall(calldatas, validation) {
    ...
    if (typeof validation === 'undefined') {
      return dswapV3Sdk.Multicall.encodeMulticall(calldatas);
    }
    ...
    if (typeof validation === 'string' && validation.startsWith('0x')) {
      ...
      var deadline = dswapV3Sdk.toHex(validation);
...
var PaymentsExtended = /*#__PURE__*/function () {
  ...
  PaymentsExtended.encodeUnwrapWETH9 = function encodeUnwrapWETH9(amountMinimum, recipient, feeOptions) {
    ...
    if (typeof recipient === 'string') {
      return dswapV3Sdk.Payments.encodeUnwrapWETH9(amountMinimum, recipient, feeOptions);
    }
    if (!!feeOptions) {
      var feeBips = encodeFeeBips(feeOptions.fee);
      var feeRecipient = dswapSdkCore.validateAndParseAddress(feeOptions.recipient);
      return PaymentsExtended.INTERFACE.encodeFunctionData('unwrapWSEIWithFee(uint256,uint256,address)', [dswapV3Sdk.toHex(amountMinimum), feeBips, feeRecipient]);
    } else {
      return PaymentsExtended.INTERFACE.encodeFunctionData('unwrapWSEI(uint256)', [dswapV3Sdk.toHex(amountMinimum)]);
    }
  };
  PaymentsExtended.encodeSweepToken = function encodeSweepToken(token, amountMinimum, recipient, feeOptions) {
    // if there's a recipient, just pass it along
    if (typeof recipient === 'string') {
      return dswapV3Sdk.Payments.encodeSweepToken(token, amountMinimum, recipient, feeOptions);
    }
    if (!!feeOptions) {
      var feeBips = encodeFeeBips(feeOptions.fee);
      var feeRecipient = dswapSdkCore.validateAndParseAddress(feeOptions.recipient);
      return PaymentsExtended.INTERFACE.encodeFunctionData('sweepTokenWithFee(address,uint256,uint256,address)', [token.address, dswapV3Sdk.toHex(amountMinimum), feeBips, feeRecipient]);
    } else {
      return PaymentsExtended.INTERFACE.encodeFunctionData('sweepToken(address,uint256)', [token.address, dswapV3Sdk.toHex(amountMinimum)]);
    }
  };
  PaymentsExtended.encodePull = function encodePull(token, amount) {
    return PaymentsExtended.INTERFACE.encodeFunctionData('pull', [token.address, dswapV3Sdk.toHex(amount)]);
  };
  PaymentsExtended.encodeWrapETH = function encodeWrapETH(amount) {
    return PaymentsExtended.INTERFACE.encodeFunctionData('wrapSEI', [dswapV3Sdk.toHex(amount)]);
  };
...
var MixedRouteSDK = /*#__PURE__*/function () {
  ...
  return _createClass(MixedRouteSDK, [{
    ...
    get: function get() {
      ...
      return this._midPrice = new dswapSdkCore.Price(this.input, this.output, price.denominator, price.numerator);
    }
  }]);
}();
 ...
 * @notice This class is functionally the same as the `Trade` class in the `@miljan9602/dswap-v3-sdk` package, aside from typing and some input validation.
 * @template TInput The input token, either Ether or an ERC-20
 * @template TOutput The output token, either Ether or an ERC-20
 * @template TTradeType The trade type, either exact input or exact output
 */
var MixedRouteTrade = /*#__PURE__*/function () {
  ...
  function MixedRouteTrade(_ref) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(routes), _step; !(_step = _iterator()).done;) {
      ...
      for (var _iterator2 = _createForOfIteratorHelperLoose(route.pools), _step2; !(_step2 = _iterator2()).done;) {
        ...
        pool instanceof dswapV3Sdk.Pool ? poolAddressSet.add(dswapV3Sdk.Pool.getAddress(pool.token0, pool.token1, pool.fee)) : poolAddressSet.add(dswapV2Sdk.Pair.getAddress(pool.token0, pool.token1));
      }
    }
    !(numPools == poolAddressSet.size) ?  invariant(false, 'POOLS_DUPLICATED')  : void 0;
    !(tradeType === dswapSdkCore.TradeType.EXACT_INPUT) ?  invariant(false, 'TRADE_TYPE')  : void 0;
    this.swaps = routes;
      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          ...
          case 12:
            ...
            break;
          case 15:
            inputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
            outputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.output, amounts[amounts.length - 1].numerator, amounts[amounts.length - 1].denominator);
      ...
      return _regeneratorRuntime().wrap(function _callee2$(_context2) {
        while (1) switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            !amount.currency.equals(route.input) ?  invariant(false, 'INPUT')  : void 0;
            inputAmount = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.input, amount.numerator, amount.denominator);
            amounts[0] = dswapSdkCore.CurrencyAmount.fromFractionalAmount(route.input.wrapped, amount.numerator, amount.denominator);
            i = 0;
          case 12:
            if (!(i < route.path.length - 1)) {
              _context2.next = 22;
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance, amountOut) {
    ...
    var slippageAdjustedAmountOut = new dswapSdkCore.Fraction(ONE).add(slippageTolerance).invert().multiply(amountOut.quotient).quotient;
    return dswapSdkCore.CurrencyAmount.fromRawAmount(amountOut.currency, slippageAdjustedAmountOut);
  }
  /**
  ...
  _proto.worstExecutionPrice = function worstExecutionPrice(slippageTolerance) {
    return new dswapSdkCore.Price(this.inputAmount.currency, this.outputAmount.currency, this.maximumAmountIn(slippageTolerance).quotient, this.minimumAmountOut(slippageTolerance).quotient);
  }
  /**
    ...
    currentPools, nextAmountIn, bestTrades) {
      ...
      return _regeneratorRuntime().wrap(function _callee3$(_context3) {
        while (1) switch (_context3.prev = _context3.next) {
          ...
          case 14:
            if (!(pool instanceof dswapV2Sdk.Pair)) {
              _context3.next = 17;
              break;
            }
          ...
          case 31:
            ...
            _context3.t1 = dswapSdkCore.sortedInsert;
            _context3.t2 = bestTrades;
            _context3.next = 36;
            return MixedRouteTrade.fromRoute(new MixedRouteSDK([].concat(currentPools, [pool]), currencyAmountIn.currency, currencyOut), currencyAmountIn, dswapSdkCore.TradeType.EXACT_INPUT);
          case 36:
  ...
  return _createClass(MixedRouteTrade, [{
    ...
    get: function get() {
      ...
      }, dswapSdkCore.CurrencyAmount.fromRawAmount(inputCurrency, 0));
    ...
    get: function get() {
      ...
      var totalOutputFromRoutes = this.swaps.map(function (_ref7) {
        ...
        return total.add(cur);
      }, dswapSdkCore.CurrencyAmount.fromRawAmount(outputCurrency, 0));
    ...
    get: function get() {
      ...
      return (_this$_executionPrice = this._executionPrice) != null ? _this$_executionPrice : this._executionPrice = new dswapSdkCore.Price(this.inputAmount.currency, this.outputAmount.currency, this.inputAmount.quotient, this.outputAmount.quotient);
    }
    /**
    ...
    get: function get() {
      ...
      var spotOutputAmount = dswapSdkCore.CurrencyAmount.fromRawAmount(this.outputAmount.currency, 0);
      ...
      this._priceImpact = new dswapSdkCore.Percent(priceImpact.numerator, priceImpact.denominator);
      return this._priceImpact;
    }
  }]);
}();
...
var Trade = /*#__PURE__*/function () {
  ...
  function Trade(_ref) {
    ...
    for (var _iterator4 = _createForOfIteratorHelperLoose(this.swaps), _step4; !(_step4 = _iterator4()).done;) {
      ...
      for (var _iterator5 = _createForOfIteratorHelperLoose(_route3.pools), _step5; !(_step5 = _iterator5()).done;) {
        ...
        if (pool instanceof dswapV3Sdk.Pool) {
          poolAddressSet.add(dswapV3Sdk.Pool.getAddress(pool.token0, pool.token1, pool.fee));
        } else if (pool instanceof dswapV2Sdk.Pair) {
          var pair = pool;
          poolAddressSet.add(dswapV2Sdk.Pair.getAddress(pair.token0, pair.token1));
        } else {
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance, amountOut) {
    ...
    !!slippageTolerance.lessThan(ZERO) ?  invariant(false, 'SLIPPAGE_TOLERANCE')  : void 0;
    if (this.tradeType === dswapSdkCore.TradeType.EXACT_OUTPUT) {
      return amountOut;
    } else {
      var slippageAdjustedAmountOut = new dswapSdkCore.Fraction(ONE).add(slippageTolerance).invert().multiply(amountOut.quotient).quotient;
      return dswapSdkCore.CurrencyAmount.fromRawAmount(amountOut.currency, slippageAdjustedAmountOut);
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance, amountIn) {
    ...
    !!slippageTolerance.lessThan(ZERO) ?  invariant(false, 'SLIPPAGE_TOLERANCE')  : void 0;
    if (this.tradeType === dswapSdkCore.TradeType.EXACT_INPUT) {
      return amountIn;
    } else {
      var slippageAdjustedAmountIn = new dswapSdkCore.Fraction(ONE).add(slippageTolerance).multiply(amountIn.quotient).quotient;
      return dswapSdkCore.CurrencyAmount.fromRawAmount(amountIn.currency, slippageAdjustedAmountIn);
    }
  }
  /**
   * Return the execution price after accounting for slippage tolerance
        ...
        while (1) switch (_context.prev = _context.next) {
          case 0:
            ...
            for (_iterator6 = _createForOfIteratorHelperLoose(v2Routes); !(_step6 = _iterator6()).done;) {
              ...
              v2Trade = new dswapV2Sdk.Trade(routev2, _amount, tradeType);
          ...
          case 5:
            ...
            _context.next = 9;
            return dswapV3Sdk.Trade.fromRoute(routev3, _amount2, tradeType);
          case 9:
      ...
      return _regeneratorRuntime().wrap(function _callee2$(_context2) {
        while (1) switch (_context2.prev = _context2.next) {
          case 0:
            ...
            v3Routes = [];
            mixedRoutes = [];
            if (!(route instanceof dswapV2Sdk.Route)) {
              _context2.next = 9;
              break;
            }
            v2Trade = new dswapV2Sdk.Trade(route, amount, tradeType);
            ...
            break;
          case 9:
            if (!(route instanceof dswapV3Sdk.Route)) {
              _context2.next = 17;
              break;
            }
            _context2.next = 12;
            return dswapV3Sdk.Trade.fromRoute(route, amount, tradeType);
          case 12:
  ...
  return _createClass(Trade, [{
    ...
    get: function get() {
      ...
      return new dswapSdkCore.Percent(inputCurrency.wrapped.sellFeeBps.toNumber(), 10000);
    }
    /**
    ...
    get: function get() {
      ...
      return new dswapSdkCore.Percent(outputCurrency.wrapped.buyFeeBps.toNumber(), 10000);
    }
    /**
    ...
    get: function get() {
      ...
      for (var _iterator9 = _createForOfIteratorHelperLoose(this.swaps), _step9; !(_step9 = _iterator9()).done;) {
        ...
        var midPrice = route.midPrice;
        var postTaxInputAmount = inputAmount.multiply(new dswapSdkCore.Fraction(ONE).subtract(this.inputTax));
      ...
      var preTaxOutputAmount = this.outputAmount.divide(new dswapSdkCore.Fraction(ONE).subtract(this.outputTax));
  ...
  var _route$pools$reduce = route.pools.reduce(function (_ref, pool, index) {
      ...
      if (index === 0) {
        return {
          ...
          path: [inputToken.address, pool instanceof dswapV3Sdk.Pool ? pool.fee : V2_FEE_PATH_PLACEHOLDER, outputToken.address]
        };
      } else {
        return {
          inputToken: outputToken,
          types: [].concat(types, ['uint24', 'address']),
          path: [].concat(path, [pool instanceof dswapV3Sdk.Pool ? pool.fee : V2_FEE_PATH_PLACEHOLDER, outputToken.address])
        };
      }
    }, {
...
var partitionMixedRouteByProtocol = function partitionMixedRouteByProtocol(route) {
  ...
  while (right < route.pools.length) {
    if (route.pools[left] instanceof dswapV3Sdk.Pool && route.pools[right] instanceof dswapV2Sdk.Pair || route.pools[left] instanceof dswapV2Sdk.Pair && route.pools[right] instanceof dswapV3Sdk.Pool) {
...
var REFUND_ETH_PRICE_IMPACT_THRESHOLD = /*#__PURE__*/new dswapSdkCore.Percent( /*#__PURE__*/JSBI.BigInt(50), /*#__PURE__*/JSBI.BigInt(100));
/**
...
var SwapRouter = /*#__PURE__*/function () {
  ...
  SwapRouter.encodeV2Swap = function encodeV2Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    var amountIn = dswapV3Sdk.toHex(trade.maximumAmountIn(options.slippageTolerance).quotient);
    var amountOut = dswapV3Sdk.toHex(trade.minimumAmountOut(options.slippageTolerance).quotient);
    var path = trade.route.path.map(function (token) {
      return token.address;
    });
    var recipient = routerMustCustody ? ADDRESS_THIS : typeof options.recipient === 'undefined' ? MSG_SENDER : dswapSdkCore.validateAndParseAddress(options.recipient);
    if (trade.tradeType === dswapSdkCore.TradeType.EXACT_INPUT) {
  ...
  SwapRouter.encodeV3Swap = function encodeV3Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    ...
    for (var _iterator = _createForOfIteratorHelperLoose(trade.swaps), _step; !(_step = _iterator()).done;) {
      ...
      var amountIn = dswapV3Sdk.toHex(trade.maximumAmountIn(options.slippageTolerance, inputAmount).quotient);
      var amountOut = dswapV3Sdk.toHex(trade.minimumAmountOut(options.slippageTolerance, outputAmount).quotient);
  ...
  SwapRouter.encodeMixedRouteSwap = function encodeMixedRouteSwap(trade, options, routerMustCustody, performAggregatedSlippageCheck) {
    var calldatas = [];
    !(trade.tradeType === dswapSdkCore.TradeType.EXACT_INPUT) ?  invariant(false, 'TRADE_TYPE')  : void 0;
  ...
  SwapRouter.encodeSwaps = function encodeSwaps(trades, options, isSwapAndAdd) {
    ...
    if (trades instanceof Trade) {
      ...
      for (var _iterator3 = _createForOfIteratorHelperLoose(trades.swaps), _step3; !(_step3 = _iterator3()).done;) {
        ...
        if (route.protocol == exports.Protocol.V2) {
          individualTrades.push(new dswapV2Sdk.Trade(route, trades.tradeType == dswapSdkCore.TradeType.EXACT_INPUT ? inputAmount : outputAmount, trades.tradeType));
        } else if (route.protocol == exports.Protocol.V3) {
          individualTrades.push(dswapV3Sdk.Trade.createUncheckedTrade({
            route: route,
            inputAmount: inputAmount,
            outputAmount: outputAmount,
    ...
    var numberOfTrades = trades.reduce(function (numberOfTrades, trade) {
      return numberOfTrades + (trade instanceof dswapV3Sdk.Trade || trade instanceof MixedRouteTrade ? trade.swaps.length : 1);
    }, 0);
    ...
    var performAggregatedSlippageCheck = sampleTrade.tradeType === dswapSdkCore.TradeType.EXACT_INPUT && numberOfTrades > 2;
    ...
    if (options.inputTokenPermit) {
      ...
      calldatas.push(dswapV3Sdk.SelfPermit.encodePermit(sampleTrade.inputAmount.currency, options.inputTokenPermit));
    }
    for (var _iterator4 = _createForOfIteratorHelperLoose(trades), _step4; !(_step4 = _iterator4()).done;) {
      var trade = _step4.value;
      if (trade instanceof dswapV2Sdk.Trade) {
        calldatas.push(SwapRouter.encodeV2Swap(trade, options, routerMustCustody, performAggregatedSlippageCheck));
      } else if (trade instanceof dswapV3Sdk.Trade) {
    ...
    var ZERO_IN = dswapSdkCore.CurrencyAmount.fromRawAmount(sampleTrade.inputAmount.currency, 0);
    var ZERO_OUT = dswapSdkCore.CurrencyAmount.fromRawAmount(sampleTrade.outputAmount.currency, 0);
  ...
  SwapRouter.swapCallParameters = function swapCallParameters(trades, options) {
    ...
    if (inputIsNative && (sampleTrade.tradeType === dswapSdkCore.TradeType.EXACT_OUTPUT || SwapRouter.riskOfPartialFill(trades))) {
      calldatas.push(dswapV3Sdk.Payments.encodeRefundETH());
    }
    return {
      calldata: MulticallExtended.encodeMulticall(calldatas, options.deadlineOrPreviousBlockhash),
      value: dswapV3Sdk.toHex(inputIsNative ? totalAmountIn.quotient : ZERO$1)
    };
  }
  /**
   * Produces the on-chain method name to call and the hex encoded parameters to pass as arguments for a given trade.
  ...
  SwapRouter.swapAndAddCallParameters = function swapAndAddCallParameters(trades, options, position, addLiquidityOptions, tokenInApprovalType, tokenOutApprovalType) {
    ...
    if (options.outputTokenPermit) {
      ...
      calldatas.push(dswapV3Sdk.SelfPermit.encodePermit(quoteAmountOut.currency, options.outputTokenPermit));
    }
    ...
    var tokenIn = inputIsNative ? dswapSdkCore.WETH9[chainId] : positionAmountIn.currency.wrapped;
    var tokenOut = outputIsNative ? dswapSdkCore.WETH9[chainId] : positionAmountOut.currency.wrapped;
    // if swap output does not make up whole outputTokenBalanceDesired, pull in remaining tokens for adding liquidity
    var amountOutRemaining = positionAmountOut.subtract(quoteAmountOut.wrapped);
    if (amountOutRemaining.greaterThan(dswapSdkCore.CurrencyAmount.fromRawAmount(positionAmountOut.currency, 0))) {
    ...
    var minimalPosition = dswapV3Sdk.Position.fromAmounts({
      pool: position.pool,
      tickLower: position.tickLower,
      tickUpper: position.tickUpper,
  ...
  SwapRouter.v3TradeWithHighPriceImpact = function v3TradeWithHighPriceImpact(trade) {
    return !(trade instanceof dswapV2Sdk.Trade) && trade.priceImpact.greaterThan(REFUND_ETH_PRICE_IMPACT_THRESHOLD);
  };
  SwapRouter.getPositionAmounts = function getPositionAmounts(position, zeroForOne) {
    ...
    var currencyAmount0 = dswapSdkCore.CurrencyAmount.fromRawAmount(position.pool.token0, amount0);
    var currencyAmount1 = dswapSdkCore.CurrencyAmount.fromRawAmount(position.pool.token1, amount1);
...
//# sourceMappingURL=dswap-router-sdk.cjs.development.js.map
